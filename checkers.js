...
const moveSelectedPiece = (toRow, toCol) => {
  ...
  board[toRow][toCol] = piece;
  board[fromRow][fromCol] = null;
  jumpPath.forEach(pos => (board[pos.row][pos.col] = null, countCapturedPieced(), updateCapturedInfo()));
  maybeCrownKing(toRow, toCol);
...
const  findKingJumps = (row, col, piece, visitedSquares, originRow = null, originCol = null, allowTurn = true) => {
  const directions = [
    [-1, -1], [-1, 1],
    [1, -1], [1, 1],
  ];
...
const getJumpPath = (fr, fc, tr, tc, piece) => {
  ...
  if (!isKing(piece)) {
    ...
    if (
      ...
      isOpponent(board[jumpedRow][jumpedCol], piece)
    ) {
